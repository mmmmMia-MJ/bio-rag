# -*- coding: utf-8 -*-
"""
BioRAG核心逻辑模块（供Streamlit app.py import使用）
=====================================================
把notebook里的核心逻辑抽出来，方便Streamlit调用。
"""
...
def build_index(pdf_files: dict, client, embed_model: str,
    ...
    progress_callback: callable(current, total, msg) 用于Streamlit进度条
    """
    import numpy as np
    ...
    for i, chunk in enumerate(all_chunks):
        ...
        try:
            resp = client.embeddings.create(model=embed_model, input=safe_text)
            chunk.embedding = resp.data[0].embedding
        except Exception:
            chunk.embedding = None
        time.sleep(0.3)
        if progress_callback:
            progress_callback(i + 1, total, f"向量化 {i+1}/{total}")
...
def ask(query: str, chunks: List[Chunk], client,
    ...
    """
    返回: {answer, retrieved_chunks: [{chunk_id, source, score, text}]}
    """
